          ...
          {table.getHeaderGroups().map(headerGroup => (
            <tr key={headerGroup.id} className='border-b border-slate-500'>
              {headerGroup.headers.map(header => {
                return (
                  <th key={header.id} className='text-left bg-gray-50 rounded-md pb-2 p-4' colSpan={header.colSpan}>
                    <div
                      {...{
...
function Filter({
  ...
  return typeof firstValue === 'number' ? (
    ...
    <input
      className="w-36 border shadow rounded p-1 text-slate-800 font-medium"
